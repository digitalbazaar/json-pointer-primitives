...
export function fromJsonPointerMap({map} = {}) {
  assert(map, 'map', Map);
  // the root entry returns immediately, discarding anything beside it
...
export function resolvePointer(obj, pointer) {
  ...
  // walked here rather than via `jsonpointer.get`, which gates on `in` and
  // so resolves inherited properties
  let cursor = obj;
...
export function toJsonPointerMap({obj, flat = false} = {}) {
  ...
  // `_toPointers` returns the cursor, not the map, for a top-level array
  const map = new Map();
...
// `jsonpointer.set` assigns through `__proto__` as a final token, swapping
// the prototype of the object being rebuilt
...
function _fromPointers({map} = {}) {
  ...
  for(const [pointer, value] of map) {
    ...
    if(value instanceof Map) {
      ...
      // an array is the ordered `@context` container; both may hold maps
